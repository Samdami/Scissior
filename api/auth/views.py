...
from .serializers import USER_FIELDS, USER_REGISTRATION_FIELDS, LOGIN_FIELDS
...
auth_namespace = Namespace(
    "auth", "Authentication api endpoints namespace", path="/auth"
)

user_model = auth_namespace.model("User", USER_FIELDS)
login_model = auth_namespace.model("Login", LOGIN_FIELDS)
signup_model = auth_namespace.model("Signup", USER_REGISTRATION_FIELDS)
...
class SignUpApiView(Resource):
    ...
    def post(self):
        ...
        response = {"uuid": new_user.uuid, "email": new_user.email, "tokens": tokens}
...
class SignInApiView(Resource):
    ...
    @auth_namespace.doc(description="It allows a user to login")
    def post(self):
        """
        Generate JWT Token
        """
        ...
        if attempt_user and check_password_hash(attempt_user.password_hash, password):
            ...
            tokens = {"access_token": access_token, "refresh_token": refresh_token}
            response = {"email": attempt_user.email, "tokens": tokens}
