...
accounts_namespace = Namespace(
    "accounts", "User account api endpoints namespace", path="/account"
)
...
class RetrieveDestroyUserApiView(Resource):
    ...
    @accounts_namespace.doc(
        ...
        params={"user_id": "An ID of a user"},
    )
    @jwt_required(False)
    def get(self):
    ...
    @accounts_namespace.doc(
        ...
        params={"public_id": "The public_id of a user"},
    )
    @jwt_required(False)
    def put(self):
        user_email = get_jwt_identity()
...
class ResetPasswordRequest(Resource):
    ...
    def post(self):
        ...
        if user:
            ...
            public_id = user.uuid
            Thread(
                target=MailService.send_reset_mail,
                kwargs={"email": user.email, "token": token, "public_id": public_id},
            ).start()
        response = {
            "success": True,
